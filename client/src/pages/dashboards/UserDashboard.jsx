...
export default function UserDashboard() {
  ...
  const stats = [
    ...
    {
      label: "Cancelled",
      value: userBookings.filter((b) => b.status === "Cancelled").length,
      color: "text-danger",
  ...
  const handleRebook = (booking) => {
    const service = services.find(
      (s) => String(s.id) === String(booking.serviceId),
    );
    const provider = providers.find(
      (p) => String(p.id) === String(booking.providerId),
    );
        ...
        <aside className="w-full md:w-64 shrink-0 border-r border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 flex flex-col">
          ...
          <div className="p-6 border-b border-slate-100 dark:border-slate-700">
            <div className="flex items-center gap-3">
              {user?.avatar && !avatarError ? (
                ...
                <div className="w-12 h-12 rounded-2xl bg-primary text-white font-bold flex items-center justify-center">
              ...
              return (
                ...
                >
                  ...
                  {item.id === "favorites" && favoriteServices.length > 0 && (
                    <span
                      className={`ml-auto text-xs px-2 py-0.5 rounded-full font-bold ${activeTab === item.id ? "bg-white/20 text-white" : "bg-primary/10 text-primary"}`}
                    >
          ...
          <div className="p-4 border-t border-slate-100 dark:border-slate-700">
            ...
            <button
              ...
              className="flex items-center gap-2 w-full px-4 py-2.5 text-danger hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl text-sm font-medium transition-colors"
            >
          ...
          {activeTab === "overview" && (
            <div className="animate-fade-in space-y-6">
              <div className="flex items-center justify-between">
                ...
                <Link
                  to="/services"
                  className="hidden md:flex items-center gap-2 px-4 py-2 bg-primary text-white rounded-xl text-sm font-semibold hover:bg-indigo-700 transition-colors"
                >
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
                {stats.map((s) => (
                  <div
                    key={s.label}
                    className={`bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-card`}
                  >
                    <div
                      className={`inline-block px-2 py-1 rounded-lg text-xs font-bold mb-3 ${s.bg} ${s.color}`}
                    >
                      {s.label}
                    </div>
                    <div
                      className={`text-3xl font-heading font-bold ${s.color}`}
                    >
                      {s.value}
                    </div>
                  </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead className="bg-slate-50 dark:bg-slate-700/50">
                      <tr>
                        {["Service", "Date", "Amount", "Status", "Action"].map(
                          (h) => (
                            <th
                              key={h}
                              className="text-left px-6 py-3 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider"
                            >
                              {h}
                            </th>
                          ),
                    ...
                    <tbody className="divide-y divide-slate-50 dark:divide-slate-700">
                      {userBookings.slice(0, 5).map((b) => {
                        const svc = services.find(
                          (s) => String(s.id) === String(b.serviceId),
                        );
                        return (
                          <tr
                            key={b.id}
                ...
                {[
                  {
                    icon: "⚡",
                    label: "Electrician",
                    to: "/services?category=Electrician",
                  },
                  {
                    icon: "🔧",
                    label: "Plumber",
                    to: "/services?category=Plumber",
                  },
                  {
                    icon: "🧹",
                    label: "Cleaning",
                  ...
                  <Link
                    key={q.label}
                    to={q.to}
                    className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-card flex flex-col items-center gap-2 hover:border-primary/30 hover:-translate-y-0.5 transition-all"
                  >
            ...
            <div className="animate-fade-in space-y-6">
              ...
              {filteredBookings.length === 0 ? (
                <div className="text-center py-20">
                  ...
                  <Link
                    to="/services"
                    className="mt-4 inline-block px-6 py-2 bg-primary text-white rounded-xl text-sm font-semibold hover:bg-indigo-700"
                  >
                    Browse Services
                  </Link>
                </div>
                ...
                <div className="space-y-4">
                  {filteredBookings.map((b) => {
                    ...
                    return (
                      <div
                        key={b.id}
                        className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-card p-5 flex flex-col sm:flex-row items-start sm:items-center gap-4"
                      >
                        ...
                        <div className="flex items-center gap-3 shrink-0">
                          ...
                          {b.status === "Completed" && (
                            <button
                              onClick={() => handleRebook(b)}
                              className="flex items-center gap-1 px-3 py-1.5 text-xs font-semibold bg-primary/10 text-primary rounded-lg hover:bg-primary hover:text-white transition-colors"
                            >
                              <RefreshCw className="w-3 h-3 shrink-0" /> Rebook
                            </button>
                          )}
                          {b.status === "Completed" &&
                            !submittedServiceIds.has(String(b.serviceId)) && (
                              <button
                                onClick={() => openReviewModal(b)}
                                className="flex items-center gap-1 px-3 py-1.5 text-xs font-semibold bg-amber-50 text-amber-600 rounded-lg hover:bg-amber-500 hover:text-white transition-colors border border-amber-200"
                              >
                ...
                <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-card overflow-hidden">
                  ...
                  <div className="divide-y divide-slate-50 dark:divide-slate-700">
                    {completedBookings.map((b) => {
                      ...
                      const alreadyReviewed = submittedServiceIds.has(
                        String(b.serviceId),
                      );
                      return (
                        <div
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-slate-500">
                              {formatDate(b.date)} · {formatCurrency(b.amount)}
                            </p>
                          </div>
                          {alreadyReviewed ? (
              ...
              {reviewsLoading ? (
                ...
                completedBookings.length === 0 && (
                  <div className="text-center py-20">
                    ...
                    <p className="text-slate-500 text-sm mt-1">
                      Complete a booking first, then come back to leave a
                      review.
                    </p>
                    <Link
                      to="/services"
                ...
                <div>
                  <h2 className="font-bold text-slate-700 dark:text-slate-300 mb-4 text-sm uppercase tracking-wider">
                    Your Submitted Reviews
                  </h2>
                  <div className="space-y-4">
                    {userReviews.map((r) => {
                      const svc = services.find(
                        (s) => String(s.id) === String(r.serviceId),
                      );
                      return (
                        <div
                          key={r._id}
                          className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-card p-5"
                        >
                          <div className="flex items-start justify-between gap-4">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <p className="font-semibold text-slate-800 dark:text-white text-sm">
                                  {svc?.title || `Service #${r.serviceId}`}
                                </p>
                                <div className="flex items-center gap-1 mt-0.5">
                                  {[1, 2, 3, 4, 5].map((n) => (
                                    <Star
                                      key={n}
                                      className={`w-3.5 h-3.5 ${n <= r.rating ? "fill-amber-400 text-amber-400" : "text-slate-200 dark:text-slate-600"}`}
                                    />
                                  ))}
                            ...
                            <div className="flex flex-col items-end gap-1 shrink-0">
                              <span
                                className={`text-xs font-bold px-2.5 py-1 rounded-full ${r.approved ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400" : "bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400"}`}
                              >
            ...
            <div className="animate-fade-in space-y-6">
              ...
              {favoriteServices.length === 0 ? (
                <div className="text-center py-20">
                  ...
                  <p className="text-slate-500 text-sm mt-1">
                    Browse services and tap ❤️ to save them here.
                  </p>
                  <Link
                    to="/services"
                    className="mt-4 inline-block px-6 py-2 bg-primary text-white rounded-xl text-sm font-semibold hover:bg-indigo-700"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {favoriteServices.map((s) => {
                    const prov = providers.find(
                      (p) => String(p.id) === String(s.providerId),
                    );
                    return (
                      <div
                        key={s.id}
                        className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-card p-5 flex gap-4"
                      >
                        {s.image ? (
                          <img
                      ...
                      {userBookings.map((b) => {
                        ...
                        return (
                          <tr
                            ...
                            className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors"
                          >
                            <td className="px-6 py-4 font-bold text-xs text-blue-500">
                              {b.bookingId}
                            </td>
                            <td className="px-6 py-4 font-medium text-slate-800 dark:text-white">
                              {svc?.title}
